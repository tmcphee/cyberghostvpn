#!/bin/bash -ex
	...
	check_up() {
		ping -c1 "1.1.1.1" #Ping CloudFlare
		if [ $? -eq 0 ]; then
			return 0
		fi
		return 1
	...
	if [ ! -f "$config_ini" ]; then
		...
		if [ -n "$ACC" ] && [ -n "$PASS" ]; then
			...
			echo "[ERROR 01] Can't Login. User didn't provide login credentials. Set the ACC and PASS ENV variables and try again." 
			exit
		fi
	else
	...
	do
		...
		if [[ $(sudo cyberghostvpn --status | grep 'No VPN connections found.' | wc -l) = "1" ]]; then
			echo '[ERROR 02] VPN Connection Lost - Attempting to reconnect....'
			cyberghost_start	
		fi
